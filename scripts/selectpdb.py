...
def get_parser():
    ...
    parser.add_argument(
        "-p",
        "--pdbdir",
        help="directory of pdb files, ie. /home/user/GRAPE/foldx/")
    parser.add_argument("-l",
                        '--listfile',
                        help='list file of energies, varied from software')
    parser.add_argument("-s", "--software", help="software name: foldx, rosetta, ABACUS")
    parser.add_argument("-c", '--cutoff', help='energy score cutoff')
    parser.add_argument("-fx",
                        '--filesuffix',
                        help='file suffix, ie. 5XJH_protein_Repair.pdb')
...
def foldxfilter(foldxout, ddgcutoff):
    ...
    for pos in ddg_dict:
        for res in ddg_dict[pos]:
            if res != "ref":
                foldxselectlist.append(pos + "_" + res + "\t" +
                                       str(ddg_dict[pos][res]))
    ...
    for line in rosetta_ddg_file:
        ...
        try:
            ...
            if mutation != "description":
                ...
                ddg = float(lst[1])
                if ddg < ddgcutoff:
                    rosettaselectlist.append(number + "_" + mt + "\t" +
                                             str(ddg))
        except IndexError:
            #print(line)
            continue
...
def abacus_filter(abacusout, ddgcutoff):
    abacuslist = []
    abacusoutfile = open(abacusout)
    for line in abacusoutfile:
        if line.startswith("site"):
            ...
            if score < ddgcutoff:
                abacuslist.append(site + "_" + mutation + "\t" + str(score))
                #print(site,mutation,score)
    return abacuslist
...
if __name__ == '__main__':
    ...
    if software == "ABACUS":
        abacusout = listfile
        abacusddgcutoff = cutoff
        selectedlist = abacus_filter(abacusout, abacusddgcutoff)
        targetdir = outdir
        copyfiles(filesuffix, pdbdir, targetdir, selectedlist)
